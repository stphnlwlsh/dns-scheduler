{
  ...
  outputs = { self, nixpkgs }:
    let
      ...
      mkEnvShell = pkgs: envName: envFiles: pkgs.mkShell {
        packages = with pkgs; [
          go
          opentofu
        ];
        ...
        shellHook = ''
          ...
          if [ -z "$NEXTDNS_PROFILE_ID" ]; then
            echo "⚠️  Warning: NEXTDNS_PROFILE_ID is not set"
          fi

          if [ -z "$NEXTDNS_PROFILE_ID_2" ]; then
            echo "⚠️  Warning: NEXTDNS_PROFILE_ID_2 is not set"
          fi
    ...
    in
    {
      devShells = forEachSystem (pkgs: {
        ...
        local = mkEnvShell pkgs "local" [ ".env_common.env" ".env_local.env" ];

        # Entry point: nix develop .#prod
        prod = mkEnvShell pkgs "prod" [ ".env_common.env" ".env_prod.env" ];
